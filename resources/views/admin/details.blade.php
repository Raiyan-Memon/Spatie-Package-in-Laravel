...
<div class="container">
    ...
    <h6>
        ...
        <form class="d-inline-block" action="{{route('admin.store', $rolename->id)}}" method="POST">
            @csrf
            <input type="hidden" name="user_idforremove" id="" value="{{$show->id}}"> 
            <input type="hidden" name="rolename" id="" value="{{$rolename->name}}"> 

            <button type="submit" class="btn btn-primary btn-sm"> {{$rolename->name}}  <span class="x">&nbsp; X</span> </button>
    </form>
    ...
    @php $role = DB::select("select * from spatie.roles"); @endphp
    ...
    <form action="{{ route('admin.store') }}" method="POST">
        ...
        <input type="hidden" name="user_idforadd" id="" value="{{$show->id}}">
    </form>
